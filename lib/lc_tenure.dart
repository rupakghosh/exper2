...
class calciiii extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return calciiiistate();
  }
}

///Crafting the UI
class calciiiistate extends State<calciiii> {
            ...
            Padding(
              ...
              child: Row(
                children: <Widget>[
                  ...
                  getCalcButton('Loan Tenure', Colors.orangeAccent, calculate),
                  Container(
                    width: 10.0,
                  ),
  ...
  String calculate() {
    ...
    var emi = double.parse(emiController.text);
    var fc = double.parse(fcController.text);
    var interest_modified= (interest/12)/100;
    var hi= (emi/interest_modified);
    var hi2 = (hi-principal);
    var hif = (hi/hi2);
    var lnhi = log(hif)/log(e);
    var hi3=(1+interest_modified);
    var hif2 = log(hi3)/log(e);
    var tn=(lnhi/hif2);


    var tnfinal = double.parse((tn).toStringAsFixed(0)).toInt();

    var tnfyr= double.parse((tnfinal/12).toStringAsFixed(2));

    var tpwfc= (emi*tnfinal);
    var tp= (tpwfc+fc);
    var tip=(tpwfc-principal);
    var tip2 = double.parse((tip).toStringAsFixed(2));
    var tpwfc2 = double.parse((tp).toStringAsFixed(2));
    var result = 'Loan Tenure- $tnfinal months($tnfyr yr) \nTotal Interest Payable - $tip2 \n\Total Payment - $tpwfc2 ';
  ...
  String calculate2() {
    ...
    var tnfyr= double.parse((tnfinal).toStringAsFixed(2));
